...
Unzip it — you'll get a `mcp-keep/` folder holding the binary, its `_internal/` runtime, and `FIRST_TIME_SETUP.md`. On **Windows** there are two binaries sharing that runtime — `mcp-keep-relay.exe` (the relay) and `mcp-keep-watchdog.exe` (the crash-supervisor start-with-OS uses) — so each process shows its role by name in Task Manager. On **macOS/Linux** it's a single `mcp-keep` (mark it executable first: `chmod +x mcp-keep/mcp-keep`). You don't launch the binary by hand: it only runs the relay when started with `--serve`, and running it bare just prints a short "your MCP client starts me, nothing to run here" notice and exits — that's deliberate, so a stray double-click never spawns an invisible relay.
